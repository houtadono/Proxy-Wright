...
# ----- helpers chỉ dùng cho Windows -----
def _is_under_program_files(p: Path) -> bool:
    if not sys.platform.startswith("win"):
        return False
    p = p.resolve()
    pf = Path(os.environ.get("ProgramFiles", r"C:\Program Files")).resolve()
    pf86 = Path(os.environ.get("ProgramFiles(x86)", r"C:\Program Files (x86)")).resolve()
    return str(p).startswith(str(pf)) or str(p).startswith(str(pf86))
...
def is_portable_mode() -> bool:
    ...
    if (app_dir / ".installed").exists():
        return False
    if _is_under_program_files(app_dir):
        return False
    if not _is_elevated_windows() and _is_writable(app_dir):
        return True
