...
from services.qt_event_runner import run_gen_in_qthread
...
class SettingTab(QWidget):
    ...
    def on_check_app_update(self):
        ...
        self._check_worker = CheckUpdateWorker(UpdaterService(), accept_prerelease=False)
    ...
    def _on_check_finished(self, info, error, checking_dialog):
        ...
        if clicked == btn_view:
            import webbrowser
            url = UpdaterService().repo_web_url or "https://github.com"
            webbrowser.open(url)
            return
        ...
        self._dl_worker = DownloadInstallWorker(UpdaterService(), info)
    ...
    def on_install_clicked(self):
        ...
        # tuỳ UI của bạn: vô hiệu nút + gợi ý đang chạy
        try:
            self.btn_install.setEnabled(False)  # nếu bạn có nút riêng
            self.btn_install.setText(self.tr("Installing…"))
        except Exception:
            pass

        # chạy generator trong QThread, KHÔNG mở dialog
        self._inst_thread, self._inst_sig = run_gen_in_qthread(
            self.manager.install_browser_events,
            browser="chromium",
            with_deps=False,
        )
        # gom log nếu bạn cần (không hiển thị ra UI)
        self._inst_sig.line.connect(self._on_install_line, Qt.QueuedConnection)
        self._inst_sig.done.connect(self._on_install_done, Qt.QueuedConnection)

    def _on_install_line(self, text: str):
        # thu log để xem khi fail (tuỳ bạn có dùng hay không)
        self._install_logs.append(text)
    ...
    def _on_install_done(self, ok: bool):
        ...
        # báo kết quả bằng 1 message box rồi thôi
        if ok:
            QMessageBox.information(
                self,
                self.tr("Install Playwright Browser"),
                self.tr("Chromium installed successfully."),
            )
        else:
            # ngắn gọn; nếu muốn dài hơn thì nối "\n".join(self._install_logs)
            tail = "\n".join(self._install_logs[-15:]) if self._install_logs else ""
            QMessageBox.critical(
                self,
                self.tr("Install Playwright Browser"),
                self.tr("Install failed. See recent logs:\n\n{}").format(tail),
            )
